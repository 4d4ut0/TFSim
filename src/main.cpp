...
int sc_main(int argc, char *argv[])
{
    ...
    bool fila = false;
    ifstream inFile;
    form fm(API::make_center(1224,700));
    place plc(fm);
    place upper(fm);
    place lower(fm);
    listbox table(fm);
    ...
    clock_m_control.caption("AUTO EXEC");
    exit.caption("EXIT");
    plc["rst"] << table;
    plc["btns"] << botao << clock_control << clock_m_control << exit;
    plc["memor"] << memory;
    plc["regs"] << reg;
    plc["rob"] << rob;
    plc["instr"] << instruct;
        ...
        else
        {
            ...
            switch(c)
            {
                ...
                case 'l':
                    ...
                    break;
                default: 
                    show_message("Opção inválida",string("Opção \"") + string(argv[k]) + string("\" inválidaqqw"));
                    break;
            }
        }
    }
    ...
    clock_m_control.events().click([&]
    {
    	for(int i; i <= nCiclos; i++)
    		if(sc_is_running())
            	sc_start();
    	clock_m_control.enabled(false);
    });
